...
const analyzeFromImage = (image: HTMLImageElement): ImageAnalysisResult => {
  // 创建分析用的小尺寸 canvas（用于性能优化）
  const analysisSize = 100; // 缩小到 100x100 进行分析
...
const calculateEdgeDensity = (imageData: ImageData): number => {
  ...
  // 归一化到 0-1（假设最大梯度为 360，即 sqrt(255^2 * 2)）
  const avgGradient = edgeSum / pixelCount;
  return Math.min(1, avgGradient / 100); // 调整分母使结果更有区分度
};
...
const calculateColorComplexity = (imageData: ImageData): number => {
  ...
  // 综合颜色数量和渐变程度
  const colorCountScore = Math.min(1, colorSet.size / 500); // 假设 500 种颜色为高复杂度
  const gradientScore = Math.min(1, (gradientSum / pixelCount) / 50); // 归一化渐变分数
...
/**
 * 获取标准板子规格选项
 * 注意：为了性能考虑，UI 上限制在 104
 * 更大尺寸可通过编辑页滑动条手动调整（最大150）
 */
export const STANDARD_BOARD_SIZES = [
  { value: 32, label: '32', description: '适合简单图标' },
  { value: 42, label: '42', description: '适合简单卡通' },
  { value: 52, label: '52', description: '标准52钉小板' },
