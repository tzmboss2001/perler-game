...
const ColorReplaceModal: React.FC<ColorReplaceModalProps> = ({
  ...
  const recommendedColors = useMemo(() => {
    return allBeadColors
      .filter((color) => color.id !== currentColor.id)
      .map((color) => ({
        color,
        delta: deltaE(currentColor.hex, color.hex),
      }))
      ...
      .filter(
        (color) =>
          ...
          (color.name.toLowerCase().includes(search) ||
            color.id.toLowerCase().includes(search) ||
    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={styles.warning}>
          将替换整个作品中的 <strong>{totalCount}</strong> 颗豆子
        </div>
        ...
        <div style={styles.section}>
          <span style={styles.label}>搜索其他颜色</span>
          <div style={styles.searchBox}>
            <MagnifyingGlass size={16} style={{ color: colors.text.muted }} />
            <input
              type="text"
              placeholder="输入色号、颜色名或十六进制"
              value={searchText}
        ...
        <div style={styles.footer}>
          {(canUndoReplace || canRedoReplace) && (
            <div style={styles.historyActions}>
              <button
                style={{
                  ...styles.historyBtn,
                  ...(canUndoReplace ? {} : styles.historyBtnDisabled),
                }}
              ...
              <button
                style={{
                  ...styles.historyBtn,
                  ...(canRedoReplace ? {} : styles.historyBtnDisabled),
                }}
                onClick={onRedoReplace}
                disabled={!canRedoReplace}
              >
                重做这次换色
              </button>
            </div>
